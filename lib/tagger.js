...
export default function tagger(template, tags) {
    ...
    const schema = JSON.parse(fs.readFileSync(new URL('../data/cf_schema.json', import.meta.url)));
    ...
    for (const name of Object.keys(template.Resources)) {
        ...
        if (schema.ResourceTypes[template.Resources[name].Type].Properties.Tags.PrimitiveType === 'Json') {
            ...
            for (const oTag of tags) {
                if (typeof oTag === 'string') {
                    template.Resources[name].Properties.Tags[oTag] = { Ref: oTag };
                } else if (typeof oTag === 'object') {
                    template.Resources[name].Properties.Tags[oTag.Key] = oTag.Value;
                }
            }
        } else if (schema.ResourceTypes[template.Resources[name].Type].Properties.Tags.Type === 'List') {
            const special = [];
