...
class Credentials {
    static load(argv, opts, cb) {
        if (opts.template === undefined) opts.template = true;

        let creds = {};

        creds.repo = path.parse(path.resolve('.')).name;
        ...
        creds.sha = String(git.stdout).replace(/\n/g, '');
        ...
        fs.readFile(path.resolve(process.env.HOME, '.deployrc.json'), (err, readcreds) => {
            if (err) return cb(new Error('No creds found - run "deploy init"'));

            creds = Object.assign(creds, JSON.parse(readcreds));
